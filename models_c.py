...
#                                            ('next_open_nt_id', int),
#                                            ])
# class EarlyElement(object):
#     def __init__(self,
#                  production: Production,
#                  next_open_nt_id: int):
#         self.production = production
#         self.next_open_nt_id = next_open_nt_id
...
class DiscRNNG(nn.Module):
    ...
    def __init__(self,
        ...
        getProductionLst("/home/anh/rnng_all/rnng_self/data/train_grammar.txt")
        self._early_stack = EarlyStack()
        self._num_open_np = 0
    ...
    def _check_push_pred_np(self, prod_id: int) -> bool:
        ...
        #     return False
        #
        # # return self._check_push_pred_np_early(pred_prod)
        return self._early_stack.check_push_pred_np(len(self._buffer), prod_id)

    def _check_push_pred_np_early(self, pred_prod):
        # early condition
        if len(self._early_stack) > 0:
            ...
            if pred_prod.lhs != top_early.production.rhs[top_early.next_open_nt_id]:
                return False

            # if maximum available open_np (self.MAX_OPEN_NP - self._num_open_np) == self._num_empty_nt, must be NT -> <w> because we have to scan token in all left empty nt
    ...
    def _check_shift(self) -> bool:
        ...
        return self._early_stack.check_shift(len(self._buffer))
    ...
    def _check_reduce(self, debug=False) -> bool:
        ...
        # else:
        #     return False
        return self._early_stack.check_reduce()
    ...
    def _append_history(self, action_id: ActionId) -> None:
        ...
        self._early_stack.append_history(action_id)
    ...
    def _push_prod(self, prod_id: int):
        ...
        assert not isinstance(cur_prod, str)
        ...
        self._early_stack.push_prod(prod_id)
    ...
    def _reduce(self) -> None:
        ...
        if not is_scan_prod(open_np.production):
            ...
            label_children = [e.lhs for e in child_prod if type(e) is Production]
        ...
        # # early parsing
        # top_early = self._early_stack[-1]
        # top_early.next_open_nt_id = top_early.next_open_nt_id + 1
