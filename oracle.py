...
# from actions import GEN, NT, REDUCE, SHIFT, get_nonterm, get_word, is_gen, is_nt
from actions import REDUCE, SHIFT
from self_typing import Action, POSTag, Word, UNKTyping
...
class Oracle(metaclass=abc.ABCMeta):
    ...
    #                 raise ValueError(
    #                     'invalid {} action, please check if the actions are correct'.format(REDUCE))
    #             parent = stack.pop()
    ...
    #
    #     actions = [NT(tree.label())] #List[Action]
    #     for child in tree:
...
class DiscOracle(Oracle):
    def __init__(self,
        ...
        assert len(words) == shift_cnt
        assert len(pos_tags) == len(words)
        # assert len(pos_tags) == len(raws)
    ...
    # @property
    # def check_unk(self):
    #     return self._check_unk

    # @property
    # def unks(self) -> List[str]:
    #     return list(self._unks)
